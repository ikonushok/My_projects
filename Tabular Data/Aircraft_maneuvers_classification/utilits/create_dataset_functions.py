...
def read_data(source_root, patch):
    ...
    arr_x, arr_y = split_sequence(file[columns[1:]].values, file[columns[:1]].values, patch)
    file_list.remove(file_list[0])
    for filename in file_list:
        file = pd.read_csv(f'{source_root}/{filename}')
        columns = file.columns.tolist()
        X, Y = split_sequence(file[columns[1:]].values, file[columns[:1]].values, patch)
